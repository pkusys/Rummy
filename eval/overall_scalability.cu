...
std::vector<float*> fbin_reads(const char* fname, size_t* d_out, size_t* n_out, int slice, int scale) {
    std::vector<float*> vec(slice / scale);
    FILE* f = fopen(fname, "r");
    int d, n;
...
int main(int argc,char **argv){
    ...
    if(argc - 1 != 4){
        printf("You should at least input 4 params: the dataset name, batch size, topk and scale number\n");
        return 0;
    }
    ...
    // Add the data
    {
        ...
        std::vector<float *> xbs = fbin_reads(db.c_str(), &d2, &nb, slice, scale);
