...
/// Restrict GPU memory implementation of the GpuResources object that provides for a
/// temporary memory manager
class RestrictedGpuResourcesImpl : public GpuResources {
   public:
    RestrictedGpuResourcesImpl();

    ~RestrictedGpuResourcesImpl() override;
...
class RestrictedGpuResources : public GpuResourcesProvider {
   public:
    RestrictedGpuResources();
    ~RestrictedGpuResources() override;
   ...
   private:
    std::shared_ptr<RestrictedGpuResourcesImpl> res_;
};
