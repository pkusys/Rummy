...
constexpr size_t kMinPageSize = (size_t)512 * 1024;

/// Size above which we page copies from the CPU to GPU (non-paged
/// memory usage)
constexpr size_t kNonPinnedPageSize = (size_t)512 * 1024;

// Default size for which we page add or search
constexpr size_t kAddPageSize = (size_t)512 * 1024;
...
void GpuIndex::searchFromCpuPaged_(
    ...
    pageSizeInVecs = std::min(pageSizeInVecs, (int)(kNonPinnedPageSize/(sizeof(float) * this->d)));
