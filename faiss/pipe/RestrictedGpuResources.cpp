...
//
// RestrictedGpuResourcesImpl
//

RestrictedGpuResourcesImpl::RestrictedGpuResourcesImpl()
...
RestrictedGpuResourcesImpl::~RestrictedGpuResourcesImpl() {
    ...
    for (auto& entry : allocs_) {
        ...
        if (!map.empty()) {
            std::cerr
                    << "RestrictedGpuResources destroyed with allocations outstanding:\n"
                    << "Device " << entry.first
...
size_t RestrictedGpuResourcesImpl::getDefaultTempMemForGPU(
        int device,
        size_t requested) {
...
void RestrictedGpuResourcesImpl::noTempMemory() {
    setTempMemory(0);
}

void RestrictedGpuResourcesImpl::setTempMemory(size_t size) {
...
void RestrictedGpuResourcesImpl::setPinnedMemory(size_t size) {
...
void RestrictedGpuResourcesImpl::setDefaultStream(
        int device,
        cudaStream_t stream) {
...
void RestrictedGpuResourcesImpl::revertDefaultStream(int device) {
    if (isInitialized(device)) {
        auto it = userDefaultStreams_.find(device);
...
void RestrictedGpuResourcesImpl::setDefaultNullStreamAllDevices() {
    for (int dev = 0; dev < getNumDevices(); ++dev) {
        setDefaultStream(dev, nullptr);
    }
}

void RestrictedGpuResourcesImpl::setLogMemoryAllocations(bool enable) {
    allocLogging_ = enable;
}

bool RestrictedGpuResourcesImpl::isInitialized(int device) const {
    // Use default streams as a marker for whether or not a certain
    // device has been initialized
    return defaultStreams_.count(device) != 0;
}

void RestrictedGpuResourcesImpl::initializeForDevice(int device) {
    if (isInitialized(device)) {
        return;
    }
...
cublasHandle_t RestrictedGpuResourcesImpl::getBlasHandle(int device) {
    initializeForDevice(device);
    return blasHandles_[device];
}

cudaStream_t RestrictedGpuResourcesImpl::getDefaultStream(int device) {
    initializeForDevice(device);
...
std::vector<cudaStream_t> RestrictedGpuResourcesImpl::getAlternateStreams(
        int device) {
    initializeForDevice(device);
    return alternateStreams_[device];
}

std::pair<void*, size_t> RestrictedGpuResourcesImpl::getPinnedMemory() {
    return std::make_pair(pinnedMemAlloc_, pinnedMemAllocSize_);
}

cudaStream_t RestrictedGpuResourcesImpl::getAsyncCopyStream(int device) {
    initializeForDevice(device);
    return asyncCopyStreams_[device];
}

void* RestrictedGpuResourcesImpl::allocMemory(const AllocRequest& req) {
    ...
    if (adjReq.space == MemorySpace::Temporary) {
        ...
        if (adjReq.size > tempMem->getSizeAvailable()) {
            ...
            std::cout << "size " << adjReq.size << " (no temp space) ";
        ...
        if (err != cudaSuccess) {
            ...
            std::stringstream ss;
            ss << "RestrictedGpuResources: alloc fail " << adjReq.toString()
    ...
    if (allocLogging_) {
        std::cout << "RestrictedGpuResources: alloc ok " << adjReq.toString()
                  << " ptr 0x" << p << "\n";
    }
...
void RestrictedGpuResourcesImpl::deallocMemory(int device, void* p) {
    ...
    if (allocLogging_) {
        std::cout << "RestrictedGpuResources: dealloc " << req.toString() << "\n";
    }
...
size_t RestrictedGpuResourcesImpl::getTempMemoryAvailable(int device) const {
    FAISS_ASSERT(isInitialized(device));
...
RestrictedGpuResourcesImpl::getMemoryInfo() const {
...
//
// RestrictedGpuResources
//

RestrictedGpuResources::RestrictedGpuResources()
        : res_(new RestrictedGpuResourcesImpl) {}

RestrictedGpuResources::~RestrictedGpuResources() {}

std::shared_ptr<GpuResources> RestrictedGpuResources::getResources() {
    return res_;
}

void RestrictedGpuResources::noTempMemory() {
    res_->noTempMemory();
}

void RestrictedGpuResources::setTempMemory(size_t size) {
    res_->setTempMemory(size);
}

void RestrictedGpuResources::setPinnedMemory(size_t size) {
    res_->setPinnedMemory(size);
}

void RestrictedGpuResources::setDefaultStream(int device, cudaStream_t stream) {
    res_->setDefaultStream(device, stream);
}

void RestrictedGpuResources::revertDefaultStream(int device) {
    res_->revertDefaultStream(device);
}

void RestrictedGpuResources::setDefaultNullStreamAllDevices() {
    res_->setDefaultNullStreamAllDevices();
}

std::map<int, std::map<std::string, std::pair<int, size_t>>>
RestrictedGpuResources::getMemoryInfo() const {
    return res_->getMemoryInfo();
}

cudaStream_t RestrictedGpuResources::getDefaultStream(int device) {
    return res_->getDefaultStream(device);
}

size_t RestrictedGpuResources::getTempMemoryAvailable(int device) const {
    return res_->getTempMemoryAvailable(device);
}

void RestrictedGpuResources::syncDefaultStreamCurrentDevice() {
    res_->syncDefaultStreamCurrentDevice();
}

void RestrictedGpuResources::setLogMemoryAllocations(bool enable) {
